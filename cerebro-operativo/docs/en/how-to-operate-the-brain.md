...
Four things you will need to do and nobody should have to ask about. They are in order of
frequency: the first happens every week, the last almost never. The roles ("who approves
values", "who executes") are the ones in `_config/fuentes.md` of this folder.

The rule that runs through all four: **the brain writes, you tell it what**. Nobody edits
`playbooks/` or `decisiones/bitacora.md` by hand. It is not bureaucracy: if two people edit
the same file in Drive at the same time, one of the two versions is lost, and if someone
changes a playbook without going through the log, the record stops matching the process and
nobody knows which of the two is right anymore.
...
3. Leave it in `reuniones/entrada/`.
4. Wait for the routine's run (the time is in `_config/fuentes.md`), or ask in the project
   chat: **"process the transcript I left in entrada"**.

The brain reads it, writes the meeting file in `reuniones/`, adds the decisions to the log
and updates the playbooks that changed. The summary of what it did stays in `salidas/`.
...
## 2. Correcting a badly extracted decision
...
1. **Adds a new line** to the log that corrects the previous one. It does not delete the old
   one. That is the whole rule: in this folder nothing is deleted, because the day someone
   asks "why did this change?", the answer has to be there.
2. **Returns the playbook to the correct text**, and the wrong text moves to `## Historial`
   the way the previous one did.
...
The brain builds `casos/YYYY-client.md` with the five parts that make a case citable: what
...
## 4. What to do if the routine did not run

**How you notice:** there is no new `ingesta-YYYY-MM-DD.md` file in `salidas/`.
