...
|---------|------|---------|--------------|
| 1.1.1 | 2026-09-22 | *(this commit)* | A confirmed log line needs no playbook change or history entry. |
| 1.1.0 | 2026-09-22 | *(this commit)* | Resolves every name from the brain's vocabulary block, so it audits Spanish and English brains alike. New checks: the vocabulary block is present and complete and no folder, heading or state is named in the other language; the roles table has exactly one brain owner and no rule names a person; documents in the documents folder that no playbook, value or case cites; installation gaps and a pending kickoff meeting still open; a section edited by hand with no history or log line, reported for the owner to confirm. Never renames a folder. |
